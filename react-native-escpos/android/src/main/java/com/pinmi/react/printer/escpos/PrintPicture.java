...
public class PrintPicture {
    ...
    /**
     * Função de impressão de bitmap
     * Esta função imprime uma linha como uma imagem, portanto, o processamento não é fácil de cometer erros.
     *
     * @param mBitmap
     * @param nWidth
     * @param nMode
    ...
    public static byte[] POS_PrintBMP(Bitmap mBitmap, int nWidth, int nMode, int leftPadding) {
        // Gire primeiro o preto e o branco e chame a função para dimensionar o bitmap.
        int add = 7;
        int width = ((nWidth +  add) / 8) * 8;
        int height = mBitmap.getHeight() * width / mBitmap.getWidth();
        height = ((height + add) / 8) * 8;
        int left = leftPadding == 0 ? 0 : ((leftPadding+add) / 8) * 8;


        Bitmap rszBitmap = mBitmap;
        if (mBitmap.getWidth() != width) {
            rszBitmap = Bitmap.createScaledBitmap(mBitmap, width, height, true);
            //rszBitmap = Bitmap.createScaledBitmap(mBitmap, nWidth,  mBitmap.getHeight(), true);
        }
